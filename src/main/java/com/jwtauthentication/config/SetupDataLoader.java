...
public class SetupDataLoader implements ApplicationListener<ContextRefreshedEvent> {
    ...
    public void onApplicationEvent(final ContextRefreshedEvent event) {
        ...
        final Role adminRole = createRoleIfNotFound("ADMIN_ROLE", adminPrivileges);
        createRoleIfNotFound("USER_PM", userPrivileges);

        // == create initial user
        createUserIfNotFound("eshan", "eshan", "eshan@gmail.com", "abc123@#", new ArrayList<Role>(Arrays.asList(adminRole)));
    ...
    @Transactional
    User createUserIfNotFound(final String name, final String userName, final String email, final String password, final Collection<Role> roles) {
